...
export default function AllPropertiesCostManagement({ properties }: AllPropertiesCostManagementProps) {
  ...
  const allStats = useMemo(() => {
    const result = {
      ...
      totalNetIncome: 0 as number,
      propertyCount: (properties || []).length
    }
    
    ;(properties || []).forEach((property: any) => {
      result.totalIncome += (property?.totalIncome as number) || 0
      result.totalExpense += (property?.totalExpense as number) || 0
    })
    
    result.totalNetIncome = result.totalIncome - result.totalExpense
    return result
  }, [properties])
              ...
              {(properties || []).map((property: any) => {
                ...
                return (
                  <tr key={property.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`font-medium ${net >= 0 ? 'text-purple-600' : 'text-amber-600'}`}>
                        {formatCurrency(net)}
                      </span>
                    </td>
                    <td className="py-3 px-4">
